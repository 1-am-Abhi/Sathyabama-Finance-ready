...
const DisbursalHistory = () => {
    ...
    const filteredHistory = history.filter(req => {
        ...
        const officer = req.officerName?.toLowerCase() || '';
        return title.includes(search) || officer.includes(search);
    });
        ...
        <div className="p-6 space-y-6">
                    ...
                    <CardContent className="pt-6">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold mt-1">{filteredHistory.filter(h => new Date(h.disbursedAt).getMonth() === new Date().getMonth()).length}</p>
                            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <div className="relative flex-1 max-w-md w-full">
                    ...
                    <input
                        type="text"
                        placeholder="Search history by project or officer..."
                        ...
                        <table className="w-full">
                            <thead className="bg-slate-50 dark:bg-slate-900/50 border-y border-slate-100 dark:border-slate-800">
                                <tr className="text-left">
                                    ...
                                    <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-500">Officer</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {isLoading ? (
                                    <tr><td colSpan="4" className="px-6 py-12 text-center text-slate-400 italic">Fetching history...</td></tr>
                                ) : filteredHistory.length === 0 ? (
                                    <tr><td colSpan="4" className="px-6 py-12 text-center text-slate-400 italic">No disbursal records found matching your search.</td></tr>
                                ) : (
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-x-3 mt-1.5">
                                                        <span className="text-[10px] bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-400 font-black">
                                                            {item.paymentMode || 'CHEQUE'}
                                                        </span>
                                                        {item.chequeNumber && (
                                                            <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1">
                                                                <Hash className="w-3 h-3" /> {item.chequeNumber}
                                                            </span>
                                                        )}
                                                        {item.transactionId && (
                                                            <span className="text-[10px] text-slate-500 font-mono flex items-center gap-1">
                                                                <ArrowUpRight className="w-3 h-3" /> {item.transactionId}
                                                            </span>
                                                        )}
                                                    </div>
                                                </div>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-2">
                                                        <Users className="w-3.5 h-3.5 opacity-50" /> {item.officerName}
                                                    </span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
