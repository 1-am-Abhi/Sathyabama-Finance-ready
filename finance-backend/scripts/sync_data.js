...
async function syncData() {
  try {
    ...
    // Ensure 'Others' has a code
    await sequelize.query(`UPDATE "ResearchCenters" SET "code" = 'OTHERS' WHERE "name" = 'Others'`);

    console.log('Step 2: Populating researchCenterId in Projects...');
    // We use "centre" as the source column as it contains the codes/names
    await sequelize.query(`
      UPDATE "Projects"
    ...
    console.log('Step 3: Handling unmatched centres (linking to "Others")...');
    const [others] = await sequelize.query(`SELECT "_id" FROM "ResearchCenters" WHERE "name" = 'Others' LIMIT 1`);
    if (others && others[0]) {
        await sequelize.query(`
            UPDATE "Projects"
            SET "researchCenterId" = :othersId
            WHERE "researchCenterId" IS NULL AND "centre" IS NOT NULL
        `, { replacements: { othersId: others[0]._id } });
    }

    console.log('Step 4: Synchronizing Users table researchCenterId...');
    await sequelize.query(`
      UPDATE "Users"
      SET "researchCenterId" = (
        SELECT "_id"
